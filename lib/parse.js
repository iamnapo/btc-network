...
export default async ({ consensusH: userConsensusH = {}, chainparamsCPP: userChainParamsCPP = {} } = {}) => {
	...
	let consensusHFile = (await readFile(new URL("consensus.h", import.meta.url), "utf8")).toString();
	let chainparamsCPPFile = (await readFile(new URL("chainparams.cpp", import.meta.url), "utf8")).toString();
